...
public class ObjectControlled : MonoBehaviour
{
    ...
    public void UpdateSliders()
    {
        ...
        foreach (KeyValuePair<string, GameObject> controller in controllers)
        {
            ...
            changeSizeText.text = "" + String.Format("{0:0.00}", metrics[controller.Key]); // Atualiza o texto do slider para o novo valor

            sizeSlider = controller.Value.transform.Find("ChangeSizeSlider").GetComponent<UnityEngine.UI.Slider>();
            sizeSlider.value = float.Parse((String.Format("{0:0.00}", metrics[controller.Key]))); // Altera o slider para o novo valor
        }
    }
